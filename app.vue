<template>
          ...
          >
            ...
            <v-col cols="9" sm="8">
              <v-list-item density="compact">
                ...
                >
                  {{
                    prochain_passage.destination_display ||
                      prochain_passage.destination_name
                  }}
                </v-list-item-title>
  ...
  <v-slide-y-reverse-transition>
    <div
      class="text-caption text-white d-flex bg-grey pa-1"
      style="height: 26px; position: fixed; bottom: 0; width: 100%;"
    >
      ...
      •
      <span class="mx-2">
        <span class="hidden-sm-and-down">Données fournies par l'</span><a
          href="https://prim.iledefrance-mobilites.fr"
          class="text-white"
        >
          API Ile de France Mobilité
        </a></span>
      <v-spacer />
...
export default defineNuxtComponent({
  ...
  methods: {
    ...
    async autocomplete () {
      if (this.search && this.search.length > 2) {
        this.items = await $fetch(
          `/api/arrets?${new URLSearchParams({ search: this.search })}`
        );
      }
    },
    ...
    toggleStop (stopId: string) {
      ...
      if (i !== -1) { this.stops.splice(i, 1); } else { this.stops.push(stopId); }
      this.$router.push({
    ...
    async refresh () {
      this.prochains_passages = this.stops.length
        ? await $fetch(
